...
def test_all_socials(client: FlaskClient) -> None:
    response = client.get('/api/v1/socials')
    assert response.status_code == 200


def test_get_social(client: FlaskClient) -> None:
    social: SocialMedia = db.session.scalar(sa.select(SocialMedia).filter_by(user_id=TESTING_USER))
    response = client.get(f'/api/v1/socials/{social.uuid}')
    assert response.status_code == 200


def test_my_socials(client: FlaskClient) -> None:
    response = client.get('api/v1/me/socials')
    assert response.status_code == 200
    assert response.get_json()['user']['uuid'] == TESTING_USER


def test_get_user_socials(client: FlaskClient) -> None:
    response = client.get(f'api/v1/users/{TESTING_USER}/socials')
    assert response.status_code == 200


def test_update_my_socials(client: FlaskClient) -> None:
    payload = {
        "viber": "+99999999999",
